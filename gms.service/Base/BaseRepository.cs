...
public class BaseRepository<T> : IBaseRepository<T> where T : BaseEntity
{
    ...
    {
        if (int.TryParse(HttpContext.User.FindFirst("UserId")?.Value, out int result))
            return result;
        else
            return 0;
    }
    ...
    {
        if (int.TryParse(HttpContext.User.FindFirst("BranchId")?.Value, out int result))
            return result;
        else
            return 0;
    }
    ...
    public int GetGymId()
    {
        if (int.TryParse(HttpContext.User.FindFirst("GymId")?.Value, out int result))
            return result;
        else
            return 0;
    }
